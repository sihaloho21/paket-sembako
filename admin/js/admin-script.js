        ...
        async function updateOrderStatus(id, newStatus) {
            ...
            try {
                ...
                // SheetDB returns { "affected": 1 } on success for PATCH
                if (result.affected > 0 || response.ok) {
                    ...
                    // Update local data and re-render table immediately
        ...
        document.getElementById('tukar-poin-form').addEventListener('submit', async (e) => {
            ...
            const data = {
                ...
                poin: document.getElementById('tp-poin').value,
                stok: document.getElementById('tp-stok').value,
                gambar: document.getElementById('tp-gambar').value,
                deskripsi: document.getElementById('tp-deskripsi').value
            };
            ...
            try {
                ...
                const result = await response.json();
                if (result.created > 0) {
                    alert('Produk tukar poin berhasil ditambahkan!');
                    e.target.reset();
                    fetchTukarPoin();
                }
            } catch (error) { console.error(error); }
        ...
        async function handleDeleteTukarPoin(id) {
            if (!confirm('Hapus produk tukar poin ini?')) return;
            try {
        ...
        function loadSettings() {
            ...
            // API Settings
            document.getElementById('setting-main-api').value = config.mainApi;
            document.getElementById('setting-admin-api').value = config.adminApi;
            
            // Gajian Settings
            document.getElementById('setting-target-day').value = config.gajian.targetDay;
            document.getElementById('setting-default-markup').value = config.gajian.defaultMarkup * 100;
            
            // Markup Ranges
            const rangeContainer = document.getElementById('markup-ranges-container');
            rangeContainer.innerHTML = '';
            config.gajian.markups.forEach((m, index) => {
                addMarkupRangeRow(m.minDays, m.rate * 100);
            });
            
            // Reward Settings
            document.getElementById('setting-point-value').value = config.reward.pointValue;
            document.getElementById('setting-min-point').value = config.reward.minPoint;
            
            // Manual Overrides
            const overrideContainer = document.getElementById('point-overrides-container');
            overrideContainer.innerHTML = '';
            Object.entries(config.reward.manualOverrides).forEach(([name, points]) => {
                addPointOverrideRow(name, points);
            });
        }

        function addMarkupRangeRow(minDays = '', rate = '') {
            const container = document.getElementById('markup-ranges-container');
            const div = document.createElement('div');
            div.className = 'flex gap-2 items-center markup-range-row';
            div.innerHTML = `
                <div class="flex-1">
                    <input type="number" placeholder="Min Hari" value="${minDays}" class="w-full p-2 border rounded-lg text-sm range-min-days">
                </div>
                <div class="flex-1">
                    <input type="number" step="0.1" placeholder="Markup %" value="${rate}" class="w-full p-2 border rounded-lg text-sm range-rate">
                </div>
        ...
        function addPointOverrideRow(productName = '', points = '') {
            const container = document.getElementById('point-overrides-container');
            const div = document.createElement('div');
            div.className = 'flex gap-2 items-center point-override-row';
            div.innerHTML = `
                <div class="flex-[2]">
                    <input type="text" placeholder="Nama Produk" value="${productName}" class="w-full p-2 border rounded-lg text-sm override-name">
                </div>
                <div class="flex-1">
                    <input type="number" step="0.1" placeholder="Poin" value="${points}" class="w-full p-2 border rounded-lg text-sm override-points">
                </div>
                <button type="button" onclick="this.parentElement.remove()" class="p-2 text-red-500 hover:bg-red-50 rounded-lg">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                </button>
        ...
        function saveSettings() {
            // API Settings
            const mainApi = document.getElementById('setting-main-api').value;
            const adminApi = document.getElementById('setting-admin-api').value;
            CONFIG.setMainApiUrl(mainApi);
            CONFIG.setAdminApiUrl(adminApi);
            
            // Gajian Settings
            const targetDay = parseInt(document.getElementById('setting-target-day').value);
            const defaultMarkup = parseFloat(document.getElementById('setting-default-markup').value) / 100;
            
            const markups = [];
            document.querySelectorAll('.markup-range-row').forEach(row => {
                const minDays = parseInt(row.querySelector('.range-min-days').value);
                const rate = parseFloat(row.querySelector('.range-rate').value) / 100;
                if (!isNaN(minDays) && !isNaN(rate)) {
                    markups.push({ minDays, rate });
                }
            });
            ...
            CONFIG.setGajianConfig({
                targetDay,
                defaultMarkup,
                markups: markups.sort((a, b) => b.minDays - a.minDays)
            });
            
            // Reward Settings
            const pointValue = parseInt(document.getElementById('setting-point-value').value);
            const minPoint = parseFloat(document.getElementById('setting-min-point').value);
            
            const manualOverrides = {};
            document.querySelectorAll('.point-override-row').forEach(row => {
                const name = row.querySelector('.override-name').value;
                const points = parseFloat(row.querySelector('.override-points').value);
                if (name && !isNaN(points)) {
                    manualOverrides[name] = points;
                }
            });
