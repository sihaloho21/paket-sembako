...
function addTierInput(productId) {
    ...
    const lastPrice = currentTiers[currentTiers.length - 1]?.nextElementSibling?.value || 0;
    
    const newTier = {
        min_qty: lastMinQty + 5,
        price: Math.max(0, parseInt(lastPrice) - 100)
    };
...
function validateTiers(tiers) {
    ...
    // Sort by min_qty descending for validation
    const sorted = [...tiers].sort((a, b) => b.min_qty - a.min_qty);
    
    for (let i = 0; i < sorted.length - 1; i++) {
        // Price should decrease as min_qty increases
...
function showAdminToast(message, type = 'info') {
    ...
    toast.className = `${bgColor} text-white px-6 py-3 rounded-lg shadow-lg animate-pulse`;
...
document.addEventListener('DOMContentLoaded', () => {
    const section = document.getElementById('section-tiered-pricing');
